...
score_forecasts <- function(
  ...
  if (length(unique(joint_df$target_variable)) >1 ) {
    #splits joint_df into a data frame only containing one target variable
    joint_df_by_target <- tibble::tibble(
      target_variable = purrr::map_chr(
        unique(joint_df$target_variable),
        function(x) x
      ),
      data = purrr::map(
        unique(joint_df$target_variable),
        function(var) suppressMessages(joint_df %>% 
                                         dplyr::filter(target_variable==var))
      )
    )
    
    scores <- purrr::map_dfr(
      joint_df_by_target$data,
      function(joint_df_target) scoringutils::eval_forecasts(data = joint_df_target,
                                              by = observation_cols,
