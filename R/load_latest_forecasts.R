...
#' Please use [load_forecasts()] instead.
#'
...
load_latest_forecasts <- function(
  ...
  lifecycle::deprecate_warn("0.1.5",
    "load_latest_forecasts()",
    details =
      "This function has been superseded by the latest load_forecasts(). Please switch your code to using the new function."
  )
  ...
  if (source == "local_hub_repo") {
    ...
    forecasts <- load_latest_forecasts_repo(
    ...
    forecasts <- load_latest_forecasts_zoltar(
      models = models,
      forecast_dates = forecast_dates,
      locations = locations,
      types = types,
